...
<div class="h-full flex flex-col bg-black text-corpse">
    {#if loading}
    ...
    {:else if content}
        <!-- Compact header bar -->
        <div class="shrink-0 border-b border-corpse/20 px-4 py-3">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <h1 class="text-xl font-bold text-rust">{content.manifest.project.name}</h1>
                    <span class="text-xs text-corpse/60">v{content.manifest.project.version}</span>
                </div>
                <div class="flex items-center gap-4 text-xs text-corpse/60">
                    ...
                    {#if content.manifest.project.repo}
                        <a href={content.manifest.project.repo} target="_blank" rel="noopener noreferrer"
                           class="text-rust hover:underline hidden md:inline">
                            repo
                        </a>
                    {/if}
                </div>
        ...
        <div class="shrink-0 border-b border-corpse/20 px-4 py-3">
            <div class="flex gap-3">
                <input
                    ...
                    class="flex-1 bg-black border border-corpse/20 rounded-sm px-3 py-2 text-sm text-corpse placeholder-corpse/40 focus:outline-none focus:border-rust"
                />
                <select
                    bind:value={filterKind}
                    class="bg-black border border-corpse/20 rounded-sm px-3 py-2 text-sm text-corpse focus:outline-none focus:border-rust"
                >
        ...
        <div class="flex-1 flex flex-col lg:flex-row min-h-0">
            <!-- Symbols list -->
            <div class="flex-1 lg:w-1/2 flex flex-col min-h-0 border-r border-corpse/20 {showMobileDoc ? 'hidden lg:flex' : ''}">
                <div class="shrink-0 bg-rust/10 border-b border-corpse/20 px-4 py-2">
                    <h2 class="text-sm font-semibold text-rust">
                            ...
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="px-1.5 py-0.5 rounded text-[10px] bg-corpse/10 text-corpse/70">
                                            {symbol.kind}
                                        </span>
                                    </div>
                                    <div class="text-xs text-corpse/50 mt-1 font-mono truncate">
                                        {cleanFilePath(symbol.file)}:{symbol.line}
                                    </div>
                                </div>
                                <svg class="w-4 h-4 text-corpse/30 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex-1 lg:w-1/2 flex flex-col min-h-0 {!showMobileDoc && !selectedSymbol ? 'hidden lg:flex' : ''} {showMobileDoc ? '' : 'hidden lg:flex'}">
                <div class="shrink-0 bg-rust/10 border-b border-corpse/20 px-4 py-2 flex justify-between items-center">
                    <h2 class="text-sm font-semibold text-rust">Documentation</h2>
                    {#if selectedSymbol}
                        <button
                            onclick={clearSelection}
                            class="text-xs text-corpse/60 hover:text-corpse flex items-center gap-1"
                        >
                ...
                <div class="flex-1 overflow-y-auto p-4">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-2">
                                    <h3 class="text-lg font-semibold text-rust">{getSymbolName(selectedSymbol.id)}</h3>
                                    <span class="px-2 py-0.5 rounded text-xs bg-corpse/10 text-corpse/70">
                                        {selectedSymbol.kind}
                                    </span>
                                </div>
                                <div class="bg-black border border-corpse/20 rounded-sm p-3 mb-2">
                                    <pre class="text-xs text-corpse overflow-x-auto whitespace-pre-wrap">{selectedSymbol.signature}</pre>
                                </div>
                                <div class="text-xs text-corpse/50 font-mono">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-corpse/80 leading-relaxed">{selectedDoc.summary}</p>
                            </div>
                            ...
                            {#if selectedDoc.description}
                                <div>
                                    ...
                                    <p class="text-sm text-corpse/80 leading-relaxed">{selectedDoc.description}</p>
                                </div>
                            {/if}
                                ...
                                <div>
                                    ...
                                    <div class="space-y-2">
                                        {#each selectedDoc.parameters as param}
                                            <div class="bg-black border border-corpse/20 rounded-sm p-3">
                            ...
                            {#if selectedDoc.example}
                                <div>
                                    ...
                                    <div class="bg-black border border-corpse/20 rounded-sm p-3">
                                        <pre class="text-xs text-corpse overflow-x-auto whitespace-pre-wrap">{selectedDoc.example}</pre>
                                    </div>
                                </div>
                            {/if}
                    ...
                    {:else}
                        <div class="h-full flex items-center justify-center text-corpse/40 text-sm">
